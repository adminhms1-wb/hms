<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Occupancy Rate Report</h1>
                <p class="page-subtitle">View occupancy rates and room utilization</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-summary">
                <div class="summary-card">
                    ...
                    <p class="amount">{{ parseFloat(reportData.average_occupancy_rate || 0).toFixed(2) }}%</p>
                </div>
                <div class="summary-card">
                    <h3>Total Room Nights</h3>
                    <p class="amount">{{ reportData.total_room_nights || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Available Room Nights</h3>
                    <p class="amount">{{ reportData.available_room_nights || 0 }}</p>
                </div>
            </div>
            ...
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="day in reportData.daily_occupancy" :key="day.date">
                            <td>{{ day.date }}</td>
                            <td>{{ day.occupied_rooms }}</td>
                            <td>{{ day.available_rooms }}</td>
                            <td>{{ parseFloat(day.occupancy_rate || 0).toFixed(2) }}%</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view occupancy rate data</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/occupancy-rate', {
            params: {
                from: dateFrom.value,
                to: dateTo.value,
            }
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `occupancy-rate-report-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.daily_occupancy) {
        exportToExcel(
            reportData.value.daily_occupancy,
            `occupancy-rate-report-${dateFrom.value}-to-${dateTo.value}`,
            [
                { key: 'date', label: 'Date' },
