<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Room Type Performance Report</h1>
                <p class="page-subtitle">View performance metrics by room type</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="type in reportData.room_types" :key="type.room_type">
                            <td>{{ type.room_type }}</td>
                            <td>{{ type.bookings }}</td>
                            <td>${{ parseFloat(type.revenue || 0).toFixed(2) }}</td>
                            <td>${{ parseFloat(type.average_rate || 0).toFixed(2) }}</td>
                            <td>{{ parseFloat(type.occupancy_rate || 0).toFixed(2) }}%</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view room type performance</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/room-type-performance', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `room-type-performance-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.room_types) {
        exportToExcel(reportData.value.room_types, `room-type-performance-${dateFrom.value}-to-${dateTo.value}`, [
