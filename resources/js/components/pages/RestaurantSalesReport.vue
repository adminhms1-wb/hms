<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Restaurant Sales Report</h1>
                <p class="page-subtitle">View restaurant sales and performance</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-summary">
                <div class="summary-card">
                    <h3>Total Sales</h3>
                    <p class="amount">${{ parseFloat(reportData.total_sales || 0).toFixed(2) }}</p>
                </div>
                <div class="summary-card">
                    <h3>Total Orders</h3>
                    <p class="amount">{{ reportData.total_orders || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Average Order Value</h3>
                    <p class="amount">${{ parseFloat(reportData.average_order_value || 0).toFixed(2) }}</p>
                </div>
            </div>
            ...
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="day in reportData.daily_sales" :key="day.date">
                            <td>{{ day.date }}</td>
                            <td>${{ parseFloat(day.sales || 0).toFixed(2) }}</td>
                            <td>{{ day.orders }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="item in reportData.top_items" :key="item.item">
                            <td>{{ item.item }}</td>
                            <td>{{ item.quantity }}</td>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view restaurant sales</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/restaurant-sales', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `restaurant-sales-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.daily_sales) {
        exportToExcel(reportData.value.daily_sales, `restaurant-sales-${dateFrom.value}-to-${dateTo.value}`, [
            { key: 'date', label: 'Date' },
