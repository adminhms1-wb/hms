<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Amenities Usage Report</h1>
                <p class="page-subtitle">View amenities usage and consumption</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-summary">
                <div class="summary-card">
                    <h3>Total Usage</h3>
                    <p class="amount">{{ reportData.total_usage || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Total Revenue</h3>
                ...
                <div class="summary-card">
                    <h3>Most Used</h3>
                    <p class="amount">{{ reportData.most_used_amenity || 'N/A' }}</p>
                </div>
            </div>

            <div class="report-details">
                <h3>Amenities Usage</h3>
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="amenity in reportData.amenities" :key="amenity.amenity">
                            <td>{{ amenity.amenity }}</td>
                            <td>{{ amenity.usage_count }}</td>
                            <td>${{ parseFloat(amenity.revenue || 0).toFixed(2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view amenities usage</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/amenities-usage', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `amenities-usage-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.amenities) {
        exportToExcel(reportData.value.amenities, `amenities-usage-${dateFrom.value}-to-${dateTo.value}`, [
