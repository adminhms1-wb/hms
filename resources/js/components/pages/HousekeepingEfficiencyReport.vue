<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Housekeeping Efficiency Report</h1>
                <p class="page-subtitle">View housekeeping task completion and efficiency</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-summary">
                <div class="summary-card">
                    <h3>Total Tasks</h3>
                    <p class="amount">{{ reportData.total_tasks || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Completed Tasks</h3>
                    <p class="amount">{{ reportData.completed_tasks || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Completion Rate</h3>
                    <p class="amount">{{ parseFloat(reportData.completion_rate || 0).toFixed(2) }}%</p>
                </div>
            </div>
            ...
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="day in reportData.daily_efficiency" :key="day.date">
                            <td>{{ day.date }}</td>
                            <td>{{ day.tasks }}</td>
                            <td>{{ day.completed }}</td>
                            <td>{{ parseFloat(day.completion_rate || 0).toFixed(2) }}%</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view housekeeping efficiency</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/housekeeping-efficiency', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `housekeeping-efficiency-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.daily_efficiency) {
        exportToExcel(reportData.value.daily_efficiency, `housekeeping-efficiency-${dateFrom.value}-to-${dateTo.value}`, [
            { key: 'date', label: 'Date' },
