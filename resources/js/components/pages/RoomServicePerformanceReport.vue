<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Room Service Performance Report</h1>
                <p class="page-subtitle">View room service orders and performance</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            ...
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="day in reportData.daily_performance" :key="day.date">
                            <td>{{ day.date }}</td>
                            <td>{{ day.orders }}</td>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view room service performance</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/room-service-performance', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `room-service-performance-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.daily_performance) {
        exportToExcel(reportData.value.daily_performance, `room-service-performance-${dateFrom.value}-to-${dateTo.value}`, [
            { key: 'date', label: 'Date' },
