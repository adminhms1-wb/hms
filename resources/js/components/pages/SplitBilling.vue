<template>
    <div class="split-billing-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Split Billing</h1>
                <p class="page-subtitle">Split bills among multiple guests</p>
            </div>
            <button class="btn btn-primary" @click="openCreateModal">Create Split Bill</button>
        </div>

        <div class="filters-section">
            <div class="search-box">
                <input type="text" v-model="searchQuery" placeholder="Search..." @input="filterBills" />
            </div>
            <select v-model="statusFilter" @change="filterBills" class="status-filter">
        ...
        <div class="content-card">
            ...
            <div v-else-if="filteredBills.length === 0" class="empty-state">No split bills found</div>
            <div v-else>
                <table class="inventory-table">
                    ...
                    <tbody>
                        <tr v-for="bill in paginatedBills" :key="bill.id">
                            <td>#{{ bill.id }}</td>
                            <td>#{{ bill.folio_id }}</td>
                            <td>{{ bill.split_type }}</td>
                            <td>{{ bill.number_of_splits }}</td>
                            <td>${{ parseFloat(bill.total_amount || 0).toFixed(2) }}</td>
                            <td><span class="status-badge" :class="bill.status">{{ bill.status }}</span></td>
                            <td>
                                <button class="icon-btn" @click="openEditModal(bill)">Edit</button>
                                <button class="icon-btn icon-btn-danger" @click="confirmDelete(bill)">Delete</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div v-if="showModal" class="modal-overlay" @click="closeModal">
            <div class="modal-content" @click.stop>
                <div class="modal-header">
                    <h2>{{ editingBill ? 'Edit' : 'Create' }} Split Bill</h2>
                    <button class="modal-close" @click="closeModal">×</button>
                </div>
                <form @submit.prevent="saveBill" class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.split_type" class="form-control" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" v-model="form.number_of_splits" class="form-control" required min="2" />
                    </div>
                    <div class="form-group">
                        <label>Total Amount *</label>
                        <input type="number" step="0.01" v-model="form.total_amount" class="form-control" required />
                    </div>
                    <div class="form-group">
                        <label>Status</label>
        ...
        <div v-if="showDeleteModal" class="modal-overlay" @click="closeDeleteModal">
            <div class="modal-content" @click.stop>
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this split bill?</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" @click="closeDeleteModal">Cancel</button>
                    <button type="button" class="btn btn-danger" @click="deleteBill" :disabled="deleting">{{ deleting ? 'Deleting...' : 'Delete' }}</button>
                </div>
            </div>
        </div>
    </div>
...
const filteredBills = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        filtered = filtered.filter(b => b.id.toString().includes(query));
    }
...
const paginatedBills = computed(() => {
    ...
    return filteredBills.value.slice(start, start + itemsPerPage);
});
...
const loadBills = async () => {
    loading.value = true;
    try {
        const response = await window.axios.get('/api/split-billing');
...
const filterBills = () => { currentPage.value = 1; };
const openCreateModal = () => {
    editingBill.value = null;
    form.value = { folio_id: '', split_type: 'equal', number_of_splits: 2, total_amount: 0, status: 'pending', notes: '' };
...
const saveBill = async () => {
    ...
    try {
        if (editingBill.value) {
            ...
            await window.axios.post(`/api/split-billing/${editingBill.value.id}`, updatePayload);
            showSuccess('Split bill updated');
        } else {
            await window.axios.post('/api/split-billing', form.value);
...
const confirmDelete = (bill) => { billToDelete.value = bill; showDeleteModal.value = true; };
const deleteBill = async () => {
    if (!billToDelete.value) return;
    deleting.value = true;
    try {
        await window.axios.post(`/api/split-billing/${billToDelete.value.id}/delete`);
...
const closeModal = () => { showModal.value = false; editingBill.value = null; };
...
<style scoped>
.split-billing-page { padding: 24px; }
...
.search-box { flex: 1; position: relative; padding: 10px 16px; background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; }
