<template>
    <div class="report-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Staff Productivity Report</h1>
                <p class="page-subtitle">View staff performance and productivity metrics</p>
            </div>
        </div>
        ...
        <div class="content-card" v-if="reportData" ref="reportContent">
            <div class="report-summary">
                <div class="summary-card">
                    <h3>Total Staff</h3>
                    <p class="amount">{{ reportData.total_staff || 0 }}</p>
                </div>
                <div class="summary-card">
                    <h3>Average Attendance</h3>
                    <p class="amount">{{ parseFloat(reportData.average_attendance || 0).toFixed(2) }}%</p>
                </div>
                <div class="summary-card">
                    <h3>Tasks Completed</h3>
                    <p class="amount">{{ reportData.tasks_completed || 0 }}</p>
                </div>
            </div>
            ...
            <div class="report-details">
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        <tr v-for="staff in reportData.staff_performance" :key="staff.staff_id">
                            <td>{{ staff.staff_name }}</td>
                            <td>{{ parseFloat(staff.attendance_rate || 0).toFixed(2) }}%</td>
                            <td>{{ staff.tasks_completed }}</td>
                            <td>{{ parseFloat(staff.performance_score || 0).toFixed(2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-else class="content-card">
            <div class="empty-state">
                <p>Select date range and click "Generate Report" to view staff productivity</p>
            </div>
        </div>
    </div>
</template>
...
const generateReport = async () => {
    ...
    loading.value = true;
    try {
        const response = await window.axios.get('/api/reports/staff-productivity', {
...
const exportPDF = () => {
    if (reportContent.value) {
        exportToPDF(reportContent.value, `staff-productivity-${dateFrom.value}-to-${dateTo.value}`);
    }
};

const exportExcel = () => {
    if (reportData.value && reportData.value.staff_performance) {
        exportToExcel(reportData.value.staff_performance, `staff-productivity-${dateFrom.value}-to-${dateTo.value}`, [
