<template>
    <div class="inventory-store-page">
        <div class="page-header">
            <div>
                <h1 class="page-title">Inventory & Store Management</h1>
                <p class="page-subtitle">Manage your inventory, stock levels, and store operations</p>
            </div>
            <button 
...
export default {
    ...
    setup() {
        ...
        const fetchItems = () => {
            loading.value = true;
            try {
                const bootstrapItems = window?.__HMS_BOOTSTRAP__?.inventoryStoreItems;
                items.value = Array.isArray(bootstrapItems) ? bootstrapItems : [];
            } catch (error) {
                console.error('Error reading inventory items from bootstrap:', error);
        ...
        const saveItem = async () => {
            ...
            try {
                ...
                if (editingItem.value) {
                    ...
                    await window.axios.post(`/api/inventory-store-items/${editingItem.value.id}`, data);
                    showSuccess('Inventory item updated successfully!');
                } else {
                    // Create new item
                    await window.axios.post('/api/inventory-store-items', data);
        ...
        const deleteItem = async () => {
            ...
            deleting.value = true;
            try {
                await window.axios.post(`/api/inventory-store-items/${itemToDelete.value.id}/delete`);
